...
- 第 1 關從 10 隻魚開始，照比例增加到第 70 關的 200 隻。
- 等級會隨命中數增加，魚會變多、游得更快，速度曲線會一路推到第 70 關。
- 點中答案加的時間改成原本的 70%，讓遊戲更容易結束。
- 手機版的秒數和前十紀錄放在下方資訊區下面，往下滑可以看到。
- 點錯會扣時間，最佳分數會留在瀏覽器裡。
- 遊戲結束可以輸入名字，線上前十名會寫入 Supabase 的 `fish_game_record`。
